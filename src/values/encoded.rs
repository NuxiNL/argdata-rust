...
pub fn encoded<'d>(encoded: &'d [u8]) -> EncodedArgdata<'d, fd::NoFds> {
...
impl<'d, F: fd::ConvertFd> EncodedArgdata<'d, F> {
	fn iter_subfield_next<'a>(&'a self, tag: u8, offset: &mut usize) -> Option<Result<ArgdataRef<'a, 'd>, ReadError>> where 'd: 'a {
